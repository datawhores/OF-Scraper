...
def test_ogresponse_pinned():
    ...
    model_id = TEST_ID
    t = Post(PINNED_POSTS_EXAMPLE, model_id, username)
    assert (t.responsetype) == PINNED_POSTS_EXAMPLE.get("responseType")
...
def test_date_pinned():
    ...
    model_id = TEST_ID
    t = Post(PINNED_POSTS_EXAMPLE, model_id, username)
    assert (t.date) == PINNED_POSTS_EXAMPLE.get(
        "createdAt"
    ) or PINNED_POSTS_EXAMPLE.get("postedAt")
...
def test_pinned_text_wordtruncate(mocker):
    ...
    t = Media(None, 2, post)
    wordarray = list(filter(lambda x: len(x) != 0, re.split("( )", t.clean_text)))
    with check:
