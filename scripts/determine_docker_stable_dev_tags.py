...
def main():
    ...
    should_apply_stable_latest = False # Initialize as Python boolean False
    should_apply_dev_latest = False   # Initialize as Python boolean False
    ...
    try:
        ...
        sys.exit(1) # Exit here if API call fails
            ...
            if job['conclusion'] == 'success':
                if job['name'] == JOB_NAME_DOCKER_HUB_STABLE:
                    ...
                    if run_commit_timestamp > last_successful_dev_ghcr_commit_timestamp:
                        ...
                        print(f"DEBUG: Updated last_successful_dev_ghcr_commit_timestamp to {run_commit_timestamp} from run {run_id}", file=sys.stderr)
    ...
    if is_stable_release:
        if ((current_commit_timestamp > last_successful_stable_docker_hub_commit_timestamp) or
            ...
            print(f"DEBUG: should_apply_stable_latest set to True based on logic.", file=sys.stderr)
    else:
        print(f"DEBUG: Not a stable release, should_apply_stable_latest remains False.", file=sys.stderr)
    ...
    if is_dev_release:
        if ((current_commit_timestamp > last_successful_dev_docker_hub_commit_timestamp) or
            ...
            print(f"DEBUG: should_apply_dev_latest set to True based on logic.", file=sys.stderr)
    else:
        print(f"DEBUG: Not a dev release, should_apply_dev_latest remains False.", file=sys.stderr)


    # --- Output results using GITHUB_OUTPUT file (modern approach) ---
    # Final Python boolean values for debugging
