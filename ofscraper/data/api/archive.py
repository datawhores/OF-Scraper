...
async def get_archived_posts(model_id, username, c=None, post_id=None):
    ...
    post_id = post_id or []
    if len(post_id) == 0 or len(post_id) > of_env.getattr("MAX_ARCHIVED_INDIVIDUAL_SEARCH"):
...
async def get_split_array(model_id, username, after):
    ...
    filteredArray = list(filter(lambda x: arrow.get(x.get("created_at")).float_timestamp >= after, postsDataArray))
...
async def scrape_archived_posts(c, model_id, username, timestamp=None, required_posts=None, is_last_chunk=False, offset=False):
    ...
    try:
        ...
        if all_ghosts_found:
            log.info(f"Worker finished. Flagging {len(all_ghosts_found)} ghost archived posts.")
            await mark_post_as_deleted(post_ids=all_ghosts_found, model_id=model_id, username=username)
...
async def get_after(model_id, username):
    ...
    if len(filtered_items) == 0:
        youngest = await get_youngest_archived_date(model_id=model_id, username=username)
