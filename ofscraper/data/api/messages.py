...
async def get_after(model_id, username):
    ...
    if len(missing_items) == 0:
        log.debug("Using newest db date because all downloads in db are marked as downloaded")
        return arrow.get(
        ...
        log.debug(f"Setting date slightly before oldest missing item because {len(missing_items)} messages in db are marked as undownloaded")
