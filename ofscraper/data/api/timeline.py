...
async def get_timeline_posts(model_id, username, c=None, post_id=None):
    ...
    if len(post_id) == 0 or len(post_id) > of_env.getattr("MAX_TIMELINE_INDIVIDUAL_SEARCH"):
        splitArrays = await get_split_array(model_id, username, after)
        # ADDED: username is now passed into get_tasks
        tasks = get_tasks(splitArrays, c, model_id, username) 
...
def get_tasks(splitArrays, c, model_id, username, after):
    ...
    if float(after) < first_known_timestamp:
        log.debug(f"Spawning Bridge Worker to cover gap from {after} to {first_known_timestamp}")
        tasks.append(
            scrape_timeline_posts(
                c, model_id, username,
                timestamp=after,
                required_posts=[{"post_id": first_known_post.get("post_id"), "timestamp": first_known_timestamp}],
                offset=True,
            )
        )
    ...
    for i, chunk in enumerate(splitArrays):
        ...
        start_timestamp = chunk[0].get("created_at") if is_first_chunk else splitArrays[i - 1][-1].get("created_at")
...
async def get_after(model_id, username):
    ...
    if prechecks is not None:
        return max(arrow.get(prechecks).float_timestamp, arrow.get("2000").float_timestamp)
    ...
    if len(filtered_items) == 0:
        youngest = await get_youngest_timeline_date(model_id=model_id, username=username)
...
async def scrape_timeline_posts(c, model_id, username, timestamp, required_posts=None, is_last_chunk=False, offset=False):
    ...
    url = ep_next.format(model_id, current_timestamp) if current_timestamp else ep_init.format(model_id)
    has_more = True
    ...
    try:
        while has_more:
            async with c.requests_async(url=url) as r:
                ...
                if not is_last_chunk and len(required_ids) == 0:
                    log.debug("All required gap IDs found or resolved as ghosts. Exiting early.")
                    has_more = False
        ...
        if all_ghosts_found:
            log.info(f"Worker finished. Flagging {len(all_ghosts_found)} deleted posts in database.")
            log.debug(f"Flagging the following post_ids as deleted for {username}: {all_ghosts_found}")
            await mark_post_as_deleted(
                post_ids=all_ghosts_found, 
                model_id=model_id, 
                username=username
            )
