...
class AltDownload():
    ...
    async def alt_download(self,c, ele, username, model_id):
        ...
        audio = await self.alt_download_downloader(audio, c, ele)
        video = await self.alt_download_downloader(video, c, ele)
        ...
        return await self.handle_result_alt(
            sharedPlaceholderObj, ele, audio, video, username, model_id
        )


    async def alt_download_downloader(self,item, c, ele):
        ...
        async for _ in download_retry():
            with _:
                try:
                    ...
                    if not status:
                        try:
                            item = await self.alt_download_sendreq(item, c, ele, placeholderObj)
                        except Exception as E:
                            raise E
                    return item
                except OSError as E:
    ...
    async def alt_download_sendreq(self,item, c, ele, placeholderObj):
        try:
            _attempt = alt_attempt_get(item)
            ...
            return await self.send_req_inner(c, ele, item, placeholderObj)
        except OSError as E:
            raise E
        except Exception as E:
            raise E
    ...
    async def send_req_inner(self,c, ele, item, placeholderObj):
        total = None
        try:
            ...
            async with c.requests_async(
                ...
                if await check_forced_skip(ele, total) == 0:
                    ...
                    await self.download_fileobject_writer(total, l, ele, placeholderObj,item) 
    ...
    async def download_fileobject_writer(self,total, l, ele, placeholderObj,item):
        common_globals.log.debug(
        ...
        if total > constants.getattr("MAX_READ_SIZE"):
            await self.download_fileobject_writer_streamer(total, l, ele, placeholderObj)
        else:
            await self.download_fileobject_writer_reader(total, l, placeholderObj)
        common_globals.log.debug(
    ...
    async def download_fileobject_writer_reader(self,total, res, placeholderObj):
