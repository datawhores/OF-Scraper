...
def wrapper(funct): 
    ...
    def inner(*args,**kwargs):
        #setup
        long_message=functools.partial(handle_skip_helper,kwargs.pop("long_message",None) or get_default_instructions(funct))
        kwargs["long_instruction"]=kwargs.get("long_instruction",prompt_strings.KEY_BOARD)      
        kwargs["message"]=f"{kwargs.get('message')}" \
...
def multiline_input_prompt(*args,**kwargs):
    ...
    *args,
    mandatory=False,
        ...
        keybindings=keybindings.multiline,
    **kwargs
    )
