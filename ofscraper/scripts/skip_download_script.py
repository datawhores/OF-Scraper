...
def skip_download_script(total, ele):
    ...
    if not script_path or not os.path.exists(script_path):
        log.error(f"Download skip script path is invalid or not configured: '{script_path}'. Aborting script execution.")
    ...
    try:
        ...
        log.debug(f"Running download skip script for {ele.username} (media_id: {ele.id}, post_id: {ele.postid})")
        ...
        if stderr_output:
            log.warning(f"Download skip script stderr for {ele.username}:\n{stderr_output}")
        ...
        log.error(f"Download skip script executable not found: '{script_path}'. Please ensure the path is correct and the script is executable.")
