...
def final_script():
    ...
    if not script_path or not os.path.exists(script_path):
        log.error(f"Post script path is invalid or not configured: '{script_path}'. Aborting final script.")
        return
    ...
    # 4. Add comprehensive error handling
        ...
        log.error(f"Post script executable not found: '{script_path}'. Please ensure the path is correct and the script is executable.")
        ...
        log.critical(f"An unexpected error occurred while running final script with script '{script_path}': {e}", exc_info=True)
        log.traceback_(e)
