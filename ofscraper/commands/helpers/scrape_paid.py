...
from ofscraper.commands.helpers.strings import all_paid_download_str,metadata_str, all_paid_progress_metadata_str
...
def process_user_info_printer(value,length,count,download_progress=None,all_paid_download=None,log_progress=None):
    model_id = value["model_id"]
    username = value["username"]

    all_paid_download=all_paid_download or all_paid_download_str
    download_progress=download_progress or metadata_str
    log_progress=log_progress or all_paid_progress_metadata_str

    progress_utils.update_activity_count(
                totat=length,
                description=all_paid_download.format(username=username),
    )
    progress_utils.update_activity_task(
        description=(download_progress.format(username=username,model_id=model_id,count=count+1,length=length)
    ))
