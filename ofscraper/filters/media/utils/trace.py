...
def trace_log_media(count, media, filter_str):
    ...
    for count,ele  in enumerate(media):
        log.trace("{} current item count: {} mediaid: {} postid: {} data: {} \n\n".format(filter_str,count,ele.id,ele.postid,ele.media,ele.media))
...
def trace_log_post(count, media, filter_str):
    ...
    for count,ele  in enumerate(media):
        log.trace("{} current item  count: {} postid: {} data: {} \n\n".format(filter_str,count,ele.id,ele.post))
