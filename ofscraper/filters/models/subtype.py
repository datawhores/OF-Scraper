...
def subType(filterusername):
    ...
    log.debug(f"Renewal: {read_args.retriveArgsVManager().renewal}")
    if read_args.retriveArgsVManager().renewal:
    ...
    elif read_args.retriveArgsVManager().renewal is False:
    ...
    log.debug(f"Sub Status: {read_args.retriveArgsVManager().sub_status}")
    if read_args.retriveArgsVManager().sub_status:
    ...
    elif read_args.retriveArgsVManager().sub_status is False:
