...
def subType(filterusername):
    ...
    if args_.getargs().renewal == "active":
        filterusername = list(filter(lambda x: x.renewed is not None, filterusername))
        log.debug(f"active renewal filter username count: {len(filterusername)}")
    elif args_.getargs().renewal == "disabled":
        filterusername = list(filter(lambda x: x.renewed is None, filterusername))
    ...
    if args_.getargs().sub_status == "active":
        ...
        filterusername = list(filter(lambda x: x.active is None, filterusername))
