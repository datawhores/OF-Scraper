...
class TableRow():
    ...
    def get_styled(self,count):
        ...
        if not self._other_styled:
            other_styled_row=[Text(str(self.get_val(key)), style="italic #03AC13",overflow="fold") for key in row_names()]
            other_styled_row=[self.split_join_max_len(ele,30) for ele in other_styled_row]
        ...
        with self.mutex:
            ...
            if self._get_sorted_hash(key):
                ...
                self._sorted_rows = sorted(
                    self.table_data, key=lambda x: x.get_compare_val(key), reverse=self.reverse
                )
            elif key == "username":
                self._sorted_rows = sorted(
                    self.table_data, key=lambda x: (x.get_compare_val(key),x.get_compare_val("number")), reverse=self.reverse
                )
            elif key == "downloaded":
                self._sorted_rows = sorted(
                    self.table_data,
                ...
                self._sorted_rows = sorted(
                    self.table_data,
                    key=lambda x: int(float(x.get_compare_val(key))) if x.get_compare_val(key) != "Free" else 0,
                    reverse=self.reverse,
                )
                ...
                self._sorted_rows = sorted(
                    self.table_data,
                    key=lambda x: x.get_compare_val(key) if x.get_compare_val(key) else 0,
                    reverse=self.reverse,
                )
            elif key == "media_id":
                self._sorted_rows = sorted(
                ...
                with self.mutex:
                    ...
                    for name in row_names():
                        ...
                        try:
                            filter_rows = list(
                                filter(
                                    lambda x:self._status.validate(name,x.get_compare_val(name)),
                                    filter_rows,
                                )
                            )
                        except Exception as e:
