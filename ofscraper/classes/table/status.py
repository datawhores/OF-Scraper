...
class Status():
    ...
    def _length_helper(self,test):
        if not bool(self._status['max_length'])  and not bool(self._status['min_length']):
            return True
        elif bool(self._status['max_length']) and bool(self._status['min_length']):
            min_length=arrow.get("{hour}:{minute}:{second}".format(hour=self._status['min_length'].get('min_hour','0'),minute=self._status['min_length'].get('min_minute','0'),second=self._status['min_length'].get('min_second','0')),["h:m:s"])
            max_length=arrow.get("{hour}:{minute}:{second}".format(hour=self._status['max_length'].get('max_hour','0'),minute=self._status['max_length'].get('max_minute','0'),second=self._status['max_length'].get('max_second','0')),["h:m:s"])
    ...
    def _date_helper(self,test):
        if not bool(self._status["mindate"]) and not bool(self._status["maxdate"]):
            ...
            return test.is_between(arrow.get(self._status["mindate"]),arrow.get(self._status["maxdate"]),bounds="[]")
