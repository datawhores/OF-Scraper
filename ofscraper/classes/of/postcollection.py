...
class PostCollection:
    ...
    def get_all_media_filtered(self) -> list:
        ...
        filtered_media = helpers.previous_download_filter(filtered_media, username=self.username, model_id=self.model_id)
    ...
    def add_posts(self, posts_or_data_list: list, actions: list[str] = None):
        ...
        """
        actions = actions or []
        if not posts_or_data_list: return

        new_posts_added = 0
        for item in posts_or_data_list:
            post_object, post_id = (item, item.id) if isinstance(item, Post) else (None, item.get("id"))
            if not post_id:
                log.warning(f"Skipping an item because it's missing an 'id': {item}")
                continue

            if post_id not in self._posts_map:
                post_object = post_object or Post(item, self.model_id, self.username, mode=self.mode)
            ...
            if 'like' in actions: existing_post.is_like_candidate = True
            if 'download' in actions: existing_post.is_download_candidate = True
            if 'text' in actions: existing_post.is_text_candidate = True
        
        if new_posts_added > 0:
            log.info(f"Added {new_posts_added} new, unique posts to the collection.")

    def prepare_like_actions(self):
        """
        Runs the detailed like preparation ONLY on posts marked as like candidates.
        """
        log.info("Preparing posts for liking on eligible posts...")
        for post in self.posts:
            if post.is_like_candidate:
    ...
    def _get_prepared_media_from_candidates(self) -> list:
        ...
        all_media = [media for post in candidate_posts for media in post.media_to_download]
    ...
    def get_media_to_download(self) -> list:
        ...
        all_media = helpers.previous_download_filter(all_media, username=self.username, model_id=self.model_id)
    ...
    def get_posts_for_text_download(self) -> list[Post]:
        ...
        """
        if "download" not in settings.get_settings().action: return []
        # if not settings.get_download_text(): return []
        
        text_candidates = [post for post in self.posts if post.is_text_candidate]
        log.info(f"Found {len(text_candidates)} posts in areas eligible for text download.")
        ...
        log.info(f"Found {len(posts_for_text)} posts for text download after final filtering.")
