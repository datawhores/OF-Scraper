...
async def process_tasks(tasks, model_id, after):
        ...
        try:
            for task in asyncio.as_completed(
                ...
                try:
                    ...
                    log.debug(f"{common_logs.PROGRESS_IDS.format('Archived')} {list(map(lambda x:x['id'],new_posts))}")
                    log.trace(f"{common_logs.PROGRESS_RAW.format('Archived')}".format( posts="\n\n".join(list(map(lambda x: f"{common_logs.RAW_INNER} {x}", new_posts)))))
    ...
    log.debug(f"{common_logs.FINAL_IDS.format('Archived')} {list(map(lambda x:x['id'],responseArray))}")
    log.trace(f"{common_logs.FINAL_RAW.format('Archived')}".format( posts="\n\n".join(list(map(lambda x: f"{common_logs.RAW_INNER} {x}", responseArray)))))
