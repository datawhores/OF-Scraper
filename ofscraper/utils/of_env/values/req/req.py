...
def load_network_config():
    ...
    if total_timeout_env is not None:
        try:
            ...
            print(f"Warning: OFSC_TOTAL_TIMEOUT environment variable '{total_timeout_env}' is not a valid integer. Using default None.")
    ...
    config['MAX_CHUNK_SIZE'] = int(os.getenv('OFSC_MAX_CHUNK_SIZE', str(1024 * 1024 * 128)))
    ...
    # Default: 15
    config['CHUNK_SIZE_UPDATE_COUNT'] = int(os.getenv('OFSC_CHUNK_SIZE_UPDATE_COUNT', '15'))
    ...
    config['MAX_READ_SIZE'] = int(os.getenv('OFSC_MAX_READ_SIZE', str(1024 * 1024 * 16)))
    ...
    # Default: 12
    config['MAX_SEMS_BATCH_DOWNLOAD'] = int(os.getenv('OFSC_MAX_SEMS_BATCH_DOWNLOAD', '12'))

    # MAX_SEMS_SINGLE_THREAD_DOWNLOAD: Maximum semaphores for single-thread downloads.
    # Default: 50
    config['MAX_SEMS_SINGLE_THREAD_DOWNLOAD'] = int(os.getenv('OFSC_MAX_SEMS_SINGLE_THREAD_DOWNLOAD', '50'))

    # SESSION_MANAGER_SYNC_SEM_DEFAULT: Default semaphore for session manager sync operations.
    # Default: 3
    config['SESSION_MANAGER_SYNC_SEM_DEFAULT'] = int(os.getenv('OFSC_SESSION_MANAGER_SYNC_SEM_DEFAULT', '3'))

    # SESSION_MANAGER_SEM_DEFAULT: Default semaphore for general session manager operations.
    # Default: 10
    config['SESSION_MANAGER_SEM_DEFAULT'] = int(os.getenv('OFSC_SESSION_MANAGER_SEM_DEFAULT', '10'))
    ...
    # Default: 2
    config['OF_MIN_WAIT_SESSION_DEFAULT'] = float(os.getenv('OFSC_MIN_WAIT_SESSION_DEFAULT', '2'))

    # OF_MAX_WAIT_SESSION_DEFAULT: Maximum wait time for session operations (seconds).
    # Default: 6
    config['OF_MAX_WAIT_SESSION_DEFAULT'] = float(os.getenv('OFSC_MAX_WAIT_SESSION_DEFAULT', '6'))

    # OF_MIN_WAIT_EXPONENTIAL_SESSION_DEFAULT: Minimum wait for exponential backoff in sessions (seconds).
    # Default: 16
    config['OF_MIN_WAIT_EXPONENTIAL_SESSION_DEFAULT'] = float(os.getenv('OFSC_MIN_WAIT_EXPONENTIAL_SESSION_DEFAULT', '16'))

    # OF_MAX_WAIT_EXPONENTIAL_SESSION_DEFAULT: Maximum wait for exponential backoff in sessions (seconds).
    # Default: 128
    config['OF_MAX_WAIT_EXPONENTIAL_SESSION_DEFAULT'] = float(os.getenv('OFSC_MAX_WAIT_EXPONENTIAL_SESSION_DEFAULT', '128'))

    # OF_NUM_RETRIES_SESSION_DEFAULT: Number of retries for session operations.
    # Default: 10
    config['OF_NUM_RETRIES_SESSION_DEFAULT'] = int(os.getenv('OFSC_NUM_RETRIES_SESSION_DEFAULT', '10'))
    ...
    # Default: 5
    config['DOWNLOAD_NUM_TRIES_REQ'] = int(os.getenv('OFSC_DOWNLOAD_NUM_TRIES_REQ', '5'))

    # DOWNLOAD_NUM_TRIES_CHECK_REQ: Number of download check retries for requests.
    # Default: 2
    config['DOWNLOAD_NUM_TRIES_CHECK_REQ'] = int(os.getenv('OFSC_DOWNLOAD_NUM_TRIES_CHECK_REQ', '2'))
