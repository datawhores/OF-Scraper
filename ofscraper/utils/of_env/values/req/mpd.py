...
def load_mpd_config():
    ...
    if mpd_total_timeout_env is not None:
        try:
            ...
            print(f"Warning: OFSC_MPD_TOTAL_TIMEOUT environment variable '{mpd_total_timeout_env}' is not a valid integer. Using default None.")
            config['MPD_TOTAL_TIMEOUT'] = None
    else:
        config['MPD_TOTAL_TIMEOUT'] = None # Explicitly set to None if not in env or invalid
    ...
    # Default: 45
    config['MPD_POOL_CONNECT_TIMEOUT'] = int(os.getenv('OFSC_MPD_POOL_CONNECT_TIMEOUT', '45'))
    ...
    # Default: 45
    config['MPD_SESSION_SLEEP_INIT'] = int(os.getenv('OFSC_MPD_SESSION_SLEEP_INIT', '45'))
    return config