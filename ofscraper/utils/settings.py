...
def setup_settings(mediatype):
    ...
    merged.cache_disabled = (
        settings.get_settings().no_cache or config_data.get_cache_mode() == "disabled"
    )
    merged.api_cached_disabled = (
        settings.get_settings().no_cache
        or settings.get_settings().no_api_cache
        or config_data.get_cache_mode() == "api_disabled"
    )
    merged.dynamic_rules = (
        settings.get_settings().dynamic_rules or config_data.get_dynamic()
    )
    merged.download_bars = (
        settings.get_settings().downloadbars or config_data.get_show_downloadprogress()
    )
    merged.discord_level = (
        settings.get_settings().discord_level or config_data.get_discord()
    )
    merged.log_level = settings.get_settings().log_level or constants.getattr(
        "DEFAULT_LOG_LEVEL"
    )
    ...
    merged.space_replacer = (
        settings.get_settings().space_replacer
        or config_data.get_spacereplacer(mediatype=mediatype)
    )
    merged.text_length = (
        settings.get_settings().text_length
    ...
    merged.download_sems = (
        settings.get_settings().downloadsem or config_data.get_download_semaphores()
    )
    merged.max_post_count = (
        settings.get_settings().max_count or config_data.get_max_post_count()
    )
    merged.mediatypes = settings.get_settings().mediatype or config_data.get_filter()
    merged.private_key = (
        settings.get_settings().private_key or config_data.get_private_key()
    )
    ...
    merged.post_script = (
        settings.get_settings().post_script or config_data.get_post_script()
    )
    merged.download_script = (
        settings.get_settings().download_script
