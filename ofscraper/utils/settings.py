...
def setup_settings(mediatype):
    ...
    merged.cache_disabled=read_args.retriveArgs().no_cache or config_data.get_cache_mode() == "disabled"
    merged.api_cached_disabled=read_args.retriveArgs().no_cache or read_args.retriveArgs().no_api_cache or config_data.get_cache_mode() == "api_disabled"
    merged.dynamic_rules=read_args.retriveArgs().dynamic_rules or config_data.get_dynamic()
    merged.download_bars=read_args.retriveArgs().downloadbars or config_data.get_show_downloadprogress()
    merged.discord_level= read_args.retriveArgs().discord_level  or config_data.get_discord()
    merged.log_level= read_args.retriveArgs().log_level or constants.getattr("DEFAULT_LOG_LEVEL")
    ...
    merged.space_replacer=read_args.retriveArgs().space_replacer or config_data.get_spacereplacer(
        mediatype=mediatype
    )
    merged.text_length=read_args.retriveArgs().text_length or config_data.get_textlength(
        mediatype=mediatype
    )
    ...
    merged.download_sems=read_args.retriveArgs().downloadsem or config_data.get_download_semaphores()
    merged.max_post_count=read_args.retriveArgs().max_count or config_data.get_max_post_count()
    merged.mediatypes=read_args.retriveArgs().mediatype or config_data.get_filter()
    merged.private_key=read_args.retriveArgs().private_key or config_data.get_private_key()
    ...
    merged.post_script=read_args.retriveArgs().post_script or config_data.get_post_script()
    merged.download_script=read_args.retriveArgs().download_script or config_data.get_post_download_script()
