...
def _load_env_file_callback(ctx, param, values): # 'value' is now 'values' (a tuple)
    ...
    for value in values:
        ...
        try:
            if file_extension == '.env':
                ...
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    if not isinstance(data, dict):
                        raise ValueError("YAML file must contain a dictionary/map at the top level.")
                os.environ.update({str(k): str(v) for k, v in data.items()}) # Ensure keys/values are strings
            elif file_extension == '.json':
                with open(file_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    if not isinstance(data, dict):
                        raise ValueError("JSON file must contain a dictionary/object at the top level.")
                os.environ.update({str(k): str(v) for k, v in data.items()}) # Ensure keys/values are strings
            else:
                data= dotenv_values(dotenv_path=file_path)
                os.environ.update({str(k): str(v) for k, v in data.items()}) #
            ...
            print(f"Error: Environment file not found: {click.format_filename(file_path)}", err=True)
            ctx.exit(1) # Exit if file not found
        except (json.JSONDecodeError, yaml.YAMLError, ValueError) as e:
            print(f"Error parsing environment file {click.format_filename(file_path)}: {e}", err=True)
            ctx.exit(1)
        except Exception as e:
            print(f"An unexpected error occurred while loading {click.format_filename(file_path)}: {e}", err=True)
            ctx.exit(1)
    print("")
