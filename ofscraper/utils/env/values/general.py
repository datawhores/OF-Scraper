...
def load_general_config():
    ...
    if supress_outputs_env:
        ...
        config['SUPRESS_OUTPUTS'] = {"CRITICAL", "ERROR", "WARNING", "OFF", "LOW", "PROMPT"}
    ...
    # Default: True
    config['SHOW_AVATAR'] = os.getenv('OFSC_SHOW_AVATAR', 'True').lower() in ('true', '1')

    # SHOW_RESULTS_LOG: Whether to show results in the log.
    # Default: True
    config['SHOW_RESULTS_LOG'] = os.getenv('OFSC_SHOW_RESULTS_LOG', 'True').lower() in ('true', '1')

    # MODEL_PRICE_PLACEHOLDER: Placeholder text for unknown model prices.
    # Default: "Unknown_Price"
    config['MODEL_PRICE_PLACEHOLDER'] = os.getenv('OFSC_MODEL_PRICE_PLACEHOLDER', 'Unknown_Price')

    # DELETED_MODEL_PLACEHOLDER: Placeholder text for deleted models.
    # Default: "modeldeleted"
    config['DELETED_MODEL_PLACEHOLDER'] = os.getenv('OFSC_DELETED_MODEL_PLACEHOLDER', 'modeldeleted')
    ...
    config['APP_TOKEN'] = os.getenv('OFSC_APP_TOKEN', '33d57ade8c02dbc5a333db99ff9ae26a')

    # CONTINUE_BOOL: General boolean flag for continuing operations.
    # Default: True
    config['CONTINUE_BOOL'] = os.getenv('OFSC_CONTINUE_BOOL', 'True').lower() in ('true', '1')

    # FILE_COUNT_PLACEHOLDER: Flag related to file count display or logic.
    # Default: True
    config['FILE_COUNT_PLACEHOLDER'] = os.getenv('OFSC_FILE_COUNT_PLACEHOLDER', 'True').lower() in ('true', '1')

    # FILTER_SELF_MEDIA: Whether to filter media belonging to self-profile.
    # Default: True
    config['FILTER_SELF_MEDIA'] = os.getenv('OFSC_FILTER_SELF_MEDIA', 'True').lower() in ('true', '1')

    # ALLOW_DUPE_MEDIA: Whether to allow duplicate media downloads.
    # Default: False
    config['ALLOW_DUPE_MEDIA'] = os.getenv('OFSC_ALLOW_DUPE_MEDIA', 'False').lower() in ('true', '1')
    ...
    # Default: 100
    config['LARGE_TRACE_CHUNK_SIZE'] = int(os.getenv('OFSC_LARGE_TRACE_CHUNK_SIZE', '100'))
    ...
    if disclaimers_env:
        try:
            ...
            print(f"Warning: OFSC_DISCLAIMERS environment variable is not valid JSON. Using default disclaimers.")
            config['disclaimers'] = [
