...
def load_api_limits_config():
    ...
    # Default: 10
    config['MAX_TIMELINE_INDIVIDUAL_SEARCH'] = int(os.getenv('OFSC_MAX_TIMELINE_INDIVIDUAL_SEARCH', '10'))

    # MAX_PINNED_INDIVIDUAL_SEARCH: Max number of pinned posts to search individually.
    # Default: 10
    config['MAX_PINNED_INDIVIDUAL_SEARCH'] = int(os.getenv('OFSC_MAX_PINNED_INDIVIDUAL_SEARCH', '10'))

    # MAX_ARCHIVED_INDIVIDUAL_SEARCH: Max number of archived posts to search individually.
    # Default: 10
    config['MAX_ARCHIVED_INDIVIDUAL_SEARCH'] = int(os.getenv('OFSC_MAX_ARCHIVED_INDIVIDUAL_SEARCH', '10'))

    # MAX_MESSAGES_INDIVIDUAL_SEARCH: Max number of messages to search individually.
    # Default: 10
    config['MAX_MESSAGES_INDIVIDUAL_SEARCH'] = int(os.getenv('OFSC_MAX_MESSAGES_INDIVIDUAL_SEARCH', '10'))

    # MAX_STREAMS_INDIVIDUAL_SEARCH: Max number of streams to search individually.
    # Default: 10
    config['MAX_STREAMS_INDIVIDUAL_SEARCH'] = int(os.getenv('OFSC_MAX_STREAMS_INDIVIDUAL_SEARCH', '10'))

    # --- API Retry Counts ---
    # API_INDVIDIUAL_NUM_TRIES: Number of tries for individual API calls.
    # Default: 3
    config['API_INDVIDIUAL_NUM_TRIES'] = int(os.getenv('OFSC_API_INDIVIDUAL_NUM_TRIES', '3'))
    ...
    # Default: 80
    config['REASONABLE_MAX_PAGE_MESSAGES'] = int(os.getenv('OFSC_REASONABLE_MAX_PAGE_MESSAGES', '80'))
    ...
    if api_request_threadonly_env:
        config['API_REQUEST_THREADONLY'] = [os_name.strip() for os_name in api_request_threadonly_env.split(',')]
    else:
