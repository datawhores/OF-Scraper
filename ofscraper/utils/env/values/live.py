...
def load_live_display_config():
    ...
    # If set via environment, it will be a string; otherwise, it remains None.
    config['SUPRESS_DOWNLOAD_DISPLAY'] = os.getenv('OFSC_SUPRESS_DOWNLOAD_DISPLAY', None)

    # SUPRESS_SUBSCRIPTION_DISPLAY: Whether to suppress subscription display messages. Defaults to None.
    # If set via environment, it will be a string; otherwise, it remains None.
    config['SUPRESS_SUBSCRIPTION_DISPLAY'] = os.getenv('OFSC_SUPRESS_SUBSCRIPTION_DISPLAY', None)
    ...
    # Default: True
    config['DOWNLOAD_LIVE_DISPLAY'] = os.getenv('OFSC_DOWNLOAD_LIVE_DISPLAY', 'True').lower() in ('true', '1')
