...
def load_dynamic_rules_config():
    ...
    if dynamic_options_env:
        config['DYNAMIC_OPTIONS'] = [opt.strip() for opt in dynamic_options_env.split(',')]
    else:
        config['DYNAMIC_OPTIONS'] = [
           "manual", "generic", "xagler", "rafa", "datawhores"
        ]
    ...
    if dynamic_options_all_env:
        config['DYNAMIC_OPTIONS_ALL'] = [opt.strip() for opt in dynamic_options_all_env.split(',')]
    else:
        # Build default from base DYNAMIC_OPTIONS plus hardcoded aliases
        config['DYNAMIC_OPTIONS_ALL'] = config['DYNAMIC_OPTIONS'] + [
            "digitalcriminals", "dv", "dev", "dc", "digital", "digitals", "manual", "generic",
        ]
