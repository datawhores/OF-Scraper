...
def load_download_config():
    ...
    if config['MAXFILE_SEMAPHORE'] is not None:
        try:
            ...
            print(f"Warning: MAXFILE_SEMAPHORE environment variable '{os.getenv('MAXFILE_SEMAPHORE')}' is not a valid integer. Using default None.")
    ...
    config['SPACE_DOWNLOAD_MESSAGE'] = os.getenv('SPACE_DOWNLOAD_MESSAGE', "Skipping download because space min has been reached")
    ...
    # Default: 1.5
    config['MULTIPROGRESS_JOB_UPDATE_FREQ'] = float(os.getenv('MULTIPROGRESS_JOB_UPDATE_FREQ', '1.5'))

    # PROGRESS_JOB_UPDATE_FREQ: Update frequency for single progress jobs (seconds)
    # Default: 2.5
    config['PROGRESS_JOB_UPDATE_FREQ'] = float(os.getenv('PROGRESS_JOB_UPDATE_FREQ', '2.5'))

    # OVERALL_MULTI_PROGRESS_THREAD_SLEEP: Sleep time for overall multi-progress thread (seconds)
    # Default: 0.1
    config['OVERALL_MULTI_PROGRESS_THREAD_SLEEP'] = float(os.getenv('OVERALL_MULTI_PROGRESS_THREAD_SLEEP', '0.1'))

    # JOB_MULTI_PROGRESS_THREAD_SLEEP: Sleep time for individual job multi-progress thread (seconds)
    # Default: 0.1
    config['JOB_MULTI_PROGRESS_THREAD_SLEEP'] = float(os.getenv('JOB_MULTI_PROGRESS_THREAD_SLEEP', '0.1'))
    ...
    # Default: True
    config['DOWNLOAD_FORCE_KEY'] = os.getenv('DOWNLOAD_FORCE_KEY', 'True').lower() == 'true'
    ...
    # Default: False
    config['PROFILE_FORCE_KEY'] = os.getenv('PROFILE_FORCE_KEY', 'False').lower() == 'true'
