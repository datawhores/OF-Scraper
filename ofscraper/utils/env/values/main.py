...
def load_main_config():
    ...
    # Default: False
    config['SANITIZE_DB_DEFAULT'] = os.getenv('OFSC_SANITIZE_DB_DEFAULT', 'False').lower() in ('true', '1')
    ...
    # Default: True
    config['RESUME_DEFAULT'] = os.getenv('OFSC_RESUME_DEFAULT', 'True').lower() in ('true', '1')
    ...
    # Default: False
    config['PROGRESS_DEFAULT'] = os.getenv('OFSC_PROGRESS_DEFAULT', 'False').lower() in ('true', '1')

    # SAVE_PATH_DEFAULT: Default base path for saving downloaded content.
    # Default: pathlib.Path.home() / "Data/ofscraper"
    config['SAVE_PATH_DEFAULT'] = os.getenv('OFSC_SAVE_PATH_DEFAULT', str(pathlib.Path.home() / "Data/ofscraper"))
    ...
    # Default: True
    config['TRUNCATION_DEFAULT'] = os.getenv('OFSC_TRUNCATION_DEFAULT', 'True').lower() in ('true', '1')
    ...
    # Default: True
    config['ROTATE_DEFAULT'] = os.getenv('OFSC_ROTATE_DEFAULT', 'True').lower() in ('true', '1')

    # SYSTEM_FREEMIN_DEFAULT: Minimum free system space required (bytes/MB?).
    # Default: 0
    config['SYSTEM_FREEMIN_DEFAULT'] = int(os.getenv('OFSC_SYSTEM_FREEMIN_DEFAULT', '0'))

    # AVATAR_DEFAULT: Whether avatars are included by default.
    # Default: True
    config['AVATAR_DEFAULT'] = os.getenv('OFSC_AVATAR_DEFAULT', 'True').lower() in ('true', '1')
    

    # INFINITE_LOOP_DEFAULT: Whether infinite loop mode is enabled by default.
    # Default: False
    config['INFINITE_LOOP_DEFAULT'] = os.getenv('OFSC_INFINITE_LOOP_DEFAULT', 'False').lower() in ('true', '1')

    # ENABLE_AUTO_AFTER_DEFAULT: Whether auto-after processing is enabled by default.
    # Default: False
    config['ENABLE_AUTO_AFTER_DEFAULT'] = os.getenv('OFSC_ENABLE_AUTO_AFTER_DEFAULT', 'False').lower() in ('true', '1')
    ...
    # Default: False
    config['BLOCKED_ADS_DEFAULT'] = os.getenv('OFSC_BLOCKED_ADS_DEFAULT', 'False').lower() in ('true', '1')
    ...
    # Default: False
    config['INCLUDE_LABELS_ALL'] = os.getenv('OFSC_INCLUDE_LABELS_ALL', 'False').lower() in ('true', '1')

    # DISCORD_THREAD_OVERRIDE: Whether to override Discord threading.
    # Default: False
    config['DISCORD_THREAD_OVERRIDE'] = os.getenv('OFSC_DISCORD_THREAD_OVERRIDE', 'False').lower() in ('true', '1')

    # DISCORD_ASYNC: Whether Discord integration is asynchronous.
    # Default: False
    config['DISCORD_ASYNC'] = os.getenv('OFSC_DISCORD_ASYNC', 'False').lower() in ('true', '1')

    # USE_WIV_CACHE_KEY: Whether to use Widevine cache key.
    # Default: True
    config['USE_WIV_CACHE_KEY'] = os.getenv('OFSC_USE_WIV_CACHE_KEY', 'True').lower() in ('true', '1')
    
    # POST_DOWNLOAD_SCRIPT_DEFAULT: Default path for a script to run after download. Defaults to None.
    config['POST_DOWNLOAD_SCRIPT_DEFAULT'] = os.getenv('OFSC_POST_DOWNLOAD_SCRIPT_DEFAULT', None)
    ...
    config['skip_download_script_DEFAULT'] = os.getenv('OFSC_skip_download_script_DEFAULT', None)
    ...
    # Default: 0
    config['DOWNLOAD_LIMIT_DEFAULT'] = int(os.getenv('OFSC_DOWNLOAD_LIMIT_DEFAULT', '0'))
    ...
    config['DIR_FORMAT_DEFAULT'] = os.getenv('OFSC_DIR_FORMAT_DEFAULT', '{model_username}/{responsetype}/{mediatype}/')

    # FILE_FORMAT_DEFAULT: Default file naming format string.
    # Default: "{filename}.{ext}"
    config['FILE_FORMAT_DEFAULT'] = os.getenv('OFSC_FILE_FORMAT_DEFAULT', '{filename}.{ext}')

    # METADATA_DEFAULT: Default metadata file path format.
    # Default: "{configpath}/{profile}/.data/{model_id}"
    config['METADATA_DEFAULT'] = os.getenv('OFSC_METADATA_DEFAULT', '{configpath}/{profile}/.data/{model_id}')
    ...
    if response_type_default_env:
        try:
            ...
            print(f"Warning: OFSC_RESPONSE_TYPE_DEFAULT environment variable '{response_type_default_env}' is not valid JSON. Using default dictionary.")
            config['RESPONSE_TYPE_DEFAULT'] = {
                "message": "Messages", "timeline": "Posts", "archived": "Archived",
                "paid": "Messages", "stories": "Stories", "highlights": "Stories",
                "profile": "Profile", "pinned": "Posts", "streams": "Streams",
            }
    else:
        config['RESPONSE_TYPE_DEFAULT'] = {
            "message": "Messages", "timeline": "Posts", "archived": "Archived",
    ...
    if empty_media_default_env:
        try:
            ...
            print(f"Warning: OFSC_EMPTY_MEDIA_DEFAULT environment variable '{empty_media_default_env}' is not valid JSON. Using default empty dictionary.")
