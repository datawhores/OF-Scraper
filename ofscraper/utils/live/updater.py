...
def remove_api_job_task(task):
    ...
    try: api_job_progress.remove_task(task)
...
def remove_api_task(task):
    if task is None: return
    try: api_overall_progress.remove_task(task)
    except KeyError: pass
...
def update_userlist_task(task, *args, **kwargs):
    if task is None: return
    try: userlist_overall_progress.update(task, *args, **kwargs)
    except KeyError: pass

def remove_userlist_task(task):
    if task is None: return
    try: userlist_overall_progress.remove_task(task)
    except KeyError: pass
...
def remove_download_job_task(task):
    ...
    try:
        ...
        if not settings.get_settings().download_bars: return
        if len(list(filter(lambda x: x.visible, download_job_progress.tasks))) >= min_add_visible: return
...
def remove_download_task(task):
    if task is None: return
    try: download_overall_progress.remove_task(task)
    except KeyError: pass
...
def remove_metadata_task(task):
    if task is None: return
    try: metadata_overall_progress.remove_task(task)
    except KeyError: pass
